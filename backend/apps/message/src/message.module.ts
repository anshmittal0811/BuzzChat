...
@Module({
  imports: [
    ...
    SharedModule,
    MongooseModule.forFeatureAsync([{ name: Group.name, useFactory: () => GroupSchema }, { name: GroupMember.name, useFactory: () => GroupMemberSchema }, { name: Message.name, useFactory: () => MessageSchema }, { name: User.name, useFactory: () => UserSchema }]),
  ],
